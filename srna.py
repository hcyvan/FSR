        ...
        for l in f.readlines():
            ...
            if float(pos[3]) >= cutoff:
                ...
                if len(stack)>0:
                    ...
                    e=int(stack[-1][2])
                    l=e-s
                    if sum([float(x[3]) for x in stack])/l >= cutoff*2 and l>=40 and l<=500:
                        r=[stack[0][0], str(s+1), str(e+1)]
                        if p:
                            print('\t'.join(r))
                        else:
                            result.append(r)
        ...
        with open(args.input_and_output_files) as f:
            for l in f.readlines():
                if l.startswith('#') or l.strip()=='':
                    continue
                item = l.strip().split('\t')
                print('parsing input1: {}, input2: {}'.format(item[0], item[1]))
                r=find_srna(os.path.join(args.input_file_dir,item[0]),
                            os.path.join(args.input_file_dir,item[1]),
                            args.gff,
                            int(args.cutoff))

                _save(r, os.path.join(args.output_file_dir,item[2]))
...
def scale(args):
    size_map=dict()
    with open(args.bedgraph_files) as f:
        for l in f.readlines():
            if l.startswith('#'):
                continue
            item = l.strip().split('\t')
        ...
        while len(merge)!=0:
            if tmp is None:
                ...
                if tmp[2] <= merge[0][1]:
                    ...
                    # tmp is contained by merge[0]
                    tmp[1]=merge[0][1]
                    tmp[2]=merge[0][2]
                    tmp[-1]='{};{}'.format(tmp[-1],merge[0][-1])
                    ret.append(tmp)
                    merge.pop(0)
                    if len(merge) != 0:
                        tmp=merge.pop(0)
...
def merge(args):
    ...
    for m in merge:
        name=m[4].split(';')[0]
        if args.format=='gff':
            print('{}\tSRNA\tncRNA\t{}\t{}\t0\t{}\t0\tID=srna.{};Name=srna.{};Points={}'.format(m[0],m[1],m[2],m[3],name,name,m[4]))
        else:
...
if __name__=='__main__':
    ...
    #get_candidates('./bedgraph/1218011.1.bedgraph',p=True,cutoff=100)
    #exit()
